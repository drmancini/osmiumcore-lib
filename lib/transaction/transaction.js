...
var registeredTransactionNames = Payload.constants.registeredTransactionNames;
...
var fDIP0001Active = true;
var CURRENT_VERSION = 3;
var SPECIAL_TRANSACTION_ACTIVATION_VERSION = 3;
var DEFAULT_NLOCKTIME = 0;
var MAX_DIP0001_BLOCK_SIZE = 2000000;
var MAX_BLOCK_SIZE = fDIP0001Active ? MAX_DIP0001_BLOCK_SIZE : 1000000;
...
// Fee amount for a special (future/asset) transaction, this can be changed via spork, used for estimation
Transaction.SPECIAL_FEE = 100;

// max amount of satoshis in circulation
Transaction.MAX_MONEY = 1210000 * 1e8;
...
Transaction.MAX_TX_EXTRA_PAYLOAD = 10000;

Transaction.TYPES = registeredTransactionTypes;
Transaction.NAMES = registeredTransactionNames;
...
Transaction.prototype.getSerializationError = function (opts) {
  ...
  var isMissingSignatures =
    !this.canHaveNoUtxo() && this._isMissingSignatures(opts);
...
Transaction.prototype._hasFeeError = function (opts, unspent) {
  ...
  if (!opts.disableLargeFees) {
    ...
    var maximumFee = Math.max(maxFeePerNormalTransaction, instantSendFee, this.getSpecialTxFee() + maxFeePerNormalTransaction);
...
Transaction.prototype.toObject = Transaction.prototype.toJSON =
  function toObject() {
    ...
    if (this.isSpecialTransaction()) {
      obj.type = this.type;
      obj.extraPayloadSize = this.getExtraPayloadSize();
      obj.extraPayload = Payload.serializeToBuffer(this.extraPayload).toString(
        'hex'
      );
      obj[Transaction.NAMES[this.type]] = this.extraPayload.toJSON();
    }
    return obj;
  };
...
Transaction.prototype.getSpecialTxFee = function() {
  if(this.type === registeredTransactionTypes.TRANSACTION_FUTURE 
    || this.type === registeredTransactionTypes.TRANSACTION_NEW_ASSET
    || this.type === registeredTransactionTypes.TRANSACTION_UPDATE_ASSET
    || this.type === registeredTransactionTypes.TRANSACTION_MINT_ASSET
  ) {
    var payload = this.extraPayload.toJSON();
    let thisFee = payload.fee > 0 ? payload.fee : Transaction.SPECIAL_FEE;
    return thisFee * 1e8;
  } else {
    return 0;
  }
...
Transaction.prototype.canHaveNoUtxo = function () {
  return (
    ...
    this.type === Transaction.TYPES.TRANSACTION_SUBTX_TRANSITION
  );
};
...
Transaction.prototype.getInputsHash = function () {
  // Calculate and store the inputs hash

  const inputBuffers = [];
  for (const input of this.inputs) {
    const prevTxIdBuf = Buffer.from(input.prevTxId, "hex").reverse();
    const voutBuf = Buffer.alloc(4);
    voutBuf.writeUInt32LE(input.outputIndex);
    const prevVout = Buffer.from(voutBuf, "hex");
    const inputBuf = Buffer.concat([prevTxIdBuf, prevVout]);
    inputBuffers.push(inputBuf);
  }

  const inputsBuffer = Buffer.concat(inputBuffers);
  const inputsHash = Hash.sha256sha256(inputsBuffer);
  const inputHash = inputsHash.reverse().toString("hex");
  return inputHash;
};
